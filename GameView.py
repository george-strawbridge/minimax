...
if __name__ == '__main__':
    ...
    from strategy_minimax_pruning import StrategyMinimaxPruning
    strategy = ({'r': StrategyRandom, 's': StrategyMinimaxPruning})
    g = ''
    while not g in game_state.keys():
        g = input('s to play Subtract Squareï¼Œt to play Tippy ')
    s = ''
    while not s in strategy.keys():
        s = input(
            'r for random strategy for computerï¼Œs for strong computer: '
        )
